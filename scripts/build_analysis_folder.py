...
def write_04(model_key: str):
    ...
    if model_key == "llava_med":
        md += [
            "| 순위 | 프로브 | 영향 수준 (LLaVA-Med) | MUMC 비교 |",
            "|:---:|---|---|---|",
            "| 1 | P1 이미지 변형 | 43~54% | MUMC 52~99% 대비 더 robust |",
            "| 2 | P7 촬영 기기 오인식 | 32~51% | MUMC 25~61%와 비슷한 수준 |",
            "| 3 | P3 무관한 텍스트 | 21~32% | MUMC 55~90% 대비 절반 이하 |",
            "| 4 | P5 과거 병명 | 26~31% | MUMC 33~64% 대비 더 robust |",
            "| 5 | P6 학력·직업 | 20~28% | MUMC 32~73% 대비 더 robust |",
            "| 6 | P4 인구통계 | 20~28% | MUMC 30~62% 대비 더 robust |",
            "",
            "> **LLaVA-Med은 모든 prompt-perturbation probe에서 MUMC fine-tuned baseline보다 robust합니다.** 단, baseline 정확도(<10%)가 generative 출력 한계로 매우 낮으니 closed yes/no 정확도를 함께 봐야 합니다. 거절률은 0%로 fine-tune 모델과 동일하게 위험.",
        ]
    else:
        md += [
            "| 순위 | 프로브 | 영향 수준 (BiomedCLIP) | LLaVA-Med 비교 |",
            "|:---:|---|---|---|",
            "| 1 | P1 이미지 변형 | 68~84% | LLaVA 43~54% 대비 더 fragile (image embedding 직접 의존) |",
            "| 2 | P3 무관한 텍스트 | 19~49% | LLaVA 21~32%와 비슷 |",
            "| 3 | P7 촬영 기기 | 24~49% | LLaVA 32~51%와 비슷 |",
            "| 4 | P4 인구통계 | 21~46% | LLaVA 20~28% 대비 약간 더 fragile |",
            "| 5 | P6 학력·직업 | 16~42% | LLaVA 20~28%와 비슷 |",
            "| 6 | P5 과거 병명 | 17~40% | LLaVA 26~31%와 비슷 |",
            "",
...
같은 angiography 이미지 (`med19_synpic31308_modality`), prompt만 다름:
...
진단(left common iliac artery aneurysm)은 일관되지만 modality framing은 prompt 따라 자유롭게 변함. 모델이 진실(MRI)을 알면서도 prompt 거짓 전제(\"this CT\")에 굴복하는 sycophancy 패턴.
...
- 본 분석의 LLaVA-Med은 zero-shot, MUMC는 RAD+Med2019 fine-tuned. 공정 비교 위해 LLaVA-Med도 fine-tune 후 재측정 권장.
...
- LLaVA-Med을 RAD+Med 2019으로 fine-tune해 MUMC와 동일 조건 비교
"""
    else:
        md = """# 06 — 종합 해석 및 시사점
